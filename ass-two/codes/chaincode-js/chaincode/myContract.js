...
class MyContract extends Contract{
    ...
    async AddBalance(ctx, org, amount){
        ...
        balanceBytes = await ctx.stub.getState(org);
        let balance = 0;
    ...
    async GetItem(ctx, itemName){
        const compositeKey = ctx.stub.createCompositeKey('_implicit_org', [itemName]);
        itemBytes = await ctx.stub.getPrivateData('_implicit_org', compositeKey);
        if(!itemBytes || itemBytes.length == 0)
            throw new Error(`Inventory entry not found for ${itemName}`);
